...
- Reports feature — host HTML reports in B2, gated by token. Participants see their own,
  admins see all + can upload via an Upload tab. Sandboxed iframe viewer (allow-scripts).
  Deep links ?report=NN (and ?report=DATE-NUMBER for participant reports). Needs a `reports`
  table + `is_admin` flag on allowed_emails, and endpoints /reports, /report, /admin/reports.
